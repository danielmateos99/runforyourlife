...
public class TrackingReceiver : MonoBehaviour
{
    ...
    void Update()
    {
        nose.transform.position = pose["nose"];//grabs the vector position of the nose
    ...
    protected void MessageReceived(OSCMessage message) //triggered automaticaly
    {
        ...
        for(int i=0;i<list.Count; i+=3)
        {
            ...
            if (val1.Type == OSCValueType.Float) position.x = val1.FloatValue-250; //250 in order to normalize the value into the limits.
